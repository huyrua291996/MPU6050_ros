...
class IMUNode:
    def __init__(self):
        ...
        self.pub_freq = float( rospy.get_param('~pub_freq', '10') ) # hz of imu pub
                
        # I2C Communication:
        try:
            self.sensor = AngleMeterAlpha()
            rospy.loginfo("Flusing first 50 data readings ...")
            for x in range(0, 10):
                gyro_data = self.sensor.measure()
                time.sleep(0.01)
    ...
    def timerCB(self, event):    
        ...
        try:           
            ...
            imuMsg.orientation.x = float(self.sensor.getRoll())
            imuMsg.orientation.y = float(self.sensor.getPitch())
            imuMsg.orientation.z = float(self.sensor.getYaw())
            imuMsg.angular_velocity.x = float(self.sensor.getGyroX())
            imuMsg.angular_velocity.y = float(self.sensor.getGyroY())
            imuMsg.angular_velocity.z = float(self.sensor.getGyroZ())
            imuMsg.linear_acceleration.x = float(self.sensor.getAccX())
            imuMsg.linear_acceleration.y = float(self.sensor.getAccY())
            imuMsg.linear_acceleration.z = float(self.sensor.getAccZ())
